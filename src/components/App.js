  ...
  componentDidMount() {
    ...
    firebase.auth.onAuthStateChanged((authUser) => {
      ...
      if (this.state.authUser) {
        ...
        firebase.db
          .ref(`expenseTable/${this.state.authUser.uid}`)
          .on("value", (data) => {
            if (data.val() !== null) {
              this.setState({
                expenses: data.val(),
        ...
        // get all the expenses from new table
        firebase.db
          .ref(`loanTable/${this.state.authUser.uid}`)
          .on("value", (data) => {
            if (data.val() !== null) {
              this.setState({
                loans: data.val(),
              ...
              firebase.db.ref("loans").on("value", (data) => {
                if (data.val() !== null) {
                  const eachExpense = utils.eachExpense(data.val());
                  const thisUsersLoans = utils.currentUsersExpenses(
                    eachExpense,
                    this.state.authUser
                  );
        ...
        const expensesRef = firebase.db.ref(
          `expenseTable/${this.state.authUser.uid}`
        );
        ...
        const loansRef = firebase.db.ref(
          `loanTable/${this.state.authUser.uid}`
        );
